...
def main():
    ...
    from accueil import AccueilWindow  # Fenêtre de sélection de langue
    ...
    # --- Phase 1: Sélection de la langue ---
    # Création et affichage de la fenêtre d'accueil pour choisir la langue
    accueil = AccueilWindow()
    accueil.setWindowTitle("GMAO Industrielle - Sélection de la langue")
    accueil.show()
    
    # Traite les événements en attente pour s'assurer que l'UI s'affiche
    # Sans cela, l'interface pourrait ne pas s'afficher immédiatement
    app.processEvents()  

    # --- Ajout : Chargement du traducteur Qt selon la langue choisie ---
    from PySide6.QtCore import QTranslator
    import os
    lang_code = app_config.language.value  # ex : "en"
    ...
    # Variable pour stocker la langue sélectionnée par l'utilisateur
    selected_lang = None
    # Définition d'une fonction de callback pour capturer le choix de langue
    # Cette fonction sera appelée quand l'utilisateur cliquera sur le bouton
    def on_gmao_clicked():
        # Utilise 'nonlocal' pour accéder à une variable du scope parent et la modifier
        # C'est une technique de fermeture (closure) en Python
        nonlocal selected_lang
        # Récupère le texte sélectionné dans la combobox de langue
        selected_lang = accueil.lang_combo.currentText()
        # Ferme la fenêtre d'accueil
        accueil.close()
    
    # Connecte le signal 'clicked' du bouton à notre fonction callback
    # Utilise le système de signals/slots de Qt pour la gestion des événements
    accueil.btn_gmao.clicked.connect(on_gmao_clicked)
    
    # Boucle d'attente pour le choix de langue
    # Cette technique permet de rester réactif aux événements tout en attendant
    while selected_lang is None:
        # Traite les événements Qt sans bloquer complètement l'application
        app.processEvents()
        # Sort de la boucle si l'utilisateur a fermé la fenêtre sans choisir
        if not accueil.isVisible():
            break
    
    # Mappage du texte affiché dans l'UI vers l'énumération Language utilisée en interne
    # Ce dictionnaire associe chaque option affichée à sa valeur interne
    lang_map = {
    ...
    # Utilise une variable globale pour rendre la langue accessible partout
    global APP_LANGUAGE
    # Récupère la valeur correspondante ou utilise FRENCH par défaut si non trouvée
    APP_LANGUAGE = lang_map.get(selected_lang, Language.FRENCH)
    # Configure la langue dans l'objet app_config pour l'application entière
    app_config.language = APP_LANGUAGE
    
    # Log de la langue sélectionnée pour le débogage
    logger.info(f"Langue sélectionnée: {selected_lang} (code: {app_config.language})")
