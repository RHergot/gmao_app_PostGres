...
# (À l'intérieur de la classe MaintenanceService)
    ...
    def update_maintenance(self, data: Dict[str, Any]) -> Maintenance:
        ...
        try:
            ...
            with db_cursor():
                ...
                # --- AJOUT : recalcul des coûts agrégés avant sauvegarde ---
                self._recalculer_couts_aggreges(existing_maintenance)
    ...
    def _recalculer_couts_aggreges(self, maintenance):
        """Recalcule et met à jour les champs financiers agrégés de la maintenance à partir des tables détaillées."""
        # Main d'oeuvre
        intervenants = self._mi_repo.get_by_maintenance_id(maintenance.id_maintenance)
        maintenance.cout_main_oeuvre = sum(i.cout_total or 0 for i in intervenants)
        # Pièces internes
        pieces = self.get_intervention_pieces_by_maintenance_id(maintenance.id_maintenance)
        maintenance.cout_pieces_internes = sum((p.cout_total or 0) for p in pieces)
        # Frais externes
        frais = self._mfe_repo.get_by_maintenance_id(maintenance.id_maintenance)
        maintenance.cout_pieces_externes = sum(f.montant_total or 0 for f in frais if getattr(f, 'type_frais', None) == 'PIECE_EXTERNE')
        maintenance.cout_autres_frais = sum(f.montant_total or 0 for f in frais if getattr(f, 'type_frais', None) != 'PIECE_EXTERNE')
        # Total général
        maintenance.cout_total = (
            (maintenance.cout_main_oeuvre or 0) +
            (maintenance.cout_pieces_internes or 0) +
            (maintenance.cout_pieces_externes or 0) +
            (maintenance.cout_autres_frais or 0)
        )
    ...
    def calculate_maintenance_cost(self, maintenance_id: int) -> dict:
        ...
        if self._mfe_repo:
            try:
                ...
                logger.error(f"Erreur lors du calcul du résumé des frais par type: {e}")
        ...
        return {
            ...
            'details_intervenants': intervenants_items,
