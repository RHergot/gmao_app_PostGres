...
def check_work_order_statuses():
    ...
    for ot in OrdreTravail.objects.all()[:10]:
        print(f"  - OT {ot.numero_ot}: {ot.statut} | {ot.description[:50] if ot.description else 'Pas de description'}...")
    ...
    if encours_count > 0:
        ...
        for ot in OrdreTravail.objects.filter(statut='EnCours')[:5]:
            ...
            site_nom = ot.machine.site.nom if ot.machine and ot.machine.site else 'Site inconnu'
            print(f"  - {ot.numero_ot}: {machine_nom} @ {site_nom} | Priorité: {ot.priorite}")
