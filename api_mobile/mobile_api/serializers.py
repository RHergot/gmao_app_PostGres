...
from .models import (
    OrdreTravail, Technicien, Piece, Utilisateur, Maintenance,
    MouvementStock, Machine, Site, Equipe, TypeMouvement
)
...
class TechnicienSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Technicien
        fields = [
            'id_technicien', 'nom', 'prenom', 'nom_complet',
            'contact', 'qualification', 'cout_horaire',
            'equipe', 'equipe_nom', 'actif'
        ]
...
class PieceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Piece
        fields = [
            'id_piece', 'nom', 'reference',
            'stock_actuel', 'stock_reserve', 'stock_disponible', 'alerte_stock',
            'stock_alerte', 'prix_unitaire', 'unite',
            'emplacement_stockage', 'categorie', 'statut', 'fournisseur_pref'
        ]
...
class MachineSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Machine
        fields = [
            'id_machine', 'nom', 'serial', 'modele',
            'site', 'site_nom', 'fabricant', 'fabricant_nom',
            'type_machine', 'type_machine_nom',
            'date_installation', 'etat', 'localisation', 'criticite'
        ]
        read_only_fields = ['id_machine', 'site_nom', 'fabricant_nom', 'type_machine_nom']


class OrdreTravailSerializer(serializers.ModelSerializer):
    """Serializer pour les ordres de travail"""
    technicien_nom = serializers.CharField(source='technicien_assigne.nom_complet', read_only=True)
    ...
    class Meta:
        model = OrdreTravail
        fields = [
            'id_ot', 'numero_ot', 'description',
            'date_creation', 'date_prevue', 'duree_prevue_min',
            'statut', 'priorite', 'type', 'urgence',
            'technicien_assigne', 'technicien_nom', 'machine', 'machine_nom', 'site_nom',
            'utilisateur_createur', 'notes_planification',
            'is_assignable', 'is_en_cours', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id_ot', 'numero_ot', 'date_creation', 'created_at', 'updated_at',
            'technicien_nom', 'machine_nom', 'site_nom', 'is_assignable', 'is_en_cours'
        ]
...
class MaintenanceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Maintenance
        fields = [
            'id_maintenance', 'ot', 'ot_numero', 'machine', 'machine_nom',
            'technicien', 'technicien_nom', 'date_debut_reelle', 'date_fin_reelle',
            'duree_calculee', 'duree_intervention_h', 'type_reel',
            'description_travaux', 'resultat', 'notes_technicien',
            'cout_main_oeuvre', 'cout_pieces_internes', 'cout_pieces_externes',
            'cout_autres_frais', 'cout_total', 'evaluation_qualite',
            'impact_production', 'created_at'
        ]
        read_only_fields = [
            'id_maintenance', 'duree_calculee', 'technicien_nom',
            'ot_numero', 'machine_nom', 'created_at'
        ]
...
class MouvementStockSerializer(serializers.ModelSerializer):
    ...
    type_mouvement_nom = serializers.CharField(source='type_mouvement.nom', read_only=True)
    utilisateur_nom = serializers.CharField(source='utilisateur.nom_complet', read_only=True)
    
    class Meta:
        model = MouvementStock
        fields = [
            'id', 'piece', 'piece_nom', 'type_mouvement', 'type_mouvement_nom',
            'quantite', 'emplacement_source', 'emplacement_destination',
            'utilisateur', 'utilisateur_nom', 'date_mouvement',
            'reference_document', 'commentaire', 'cout_unitaire',
            'cout_total', 'stock_avant', 'stock_apres', 'valide',
            'statut_mouvement'
        ]
        read_only_fields = [
            'id', 'piece_nom', 'type_mouvement_nom', 'utilisateur_nom',
            'date_mouvement', 'stock_avant', 'stock_apres'
        ]
...
class RapportInterventionSerializer(serializers.Serializer):
    ...
    pieces_utilisees = serializers.CharField(max_length=1000, required=False, allow_blank=True)
    observations = serializers.CharField(max_length=1000, required=False, allow_blank=True)
