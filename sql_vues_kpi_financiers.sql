...
SELECT 
    ...
    SUM(CASE WHEN date_fin_reelle >= CURRENT_DATE - INTERVAL '6 months' THEN cout_total ELSE 0 END) AS cout_6m_recents,
    SUM(CASE WHEN date_fin_reelle >= CURRENT_DATE - INTERVAL '12 months' 
                  AND date_fin_reelle < CURRENT_DATE - INTERVAL '6 months' 
             THEN cout_total ELSE 0 END) AS cout_6m_precedents,
    
    CASE 
        WHEN SUM(CASE WHEN date_fin_reelle >= CURRENT_DATE - INTERVAL '12 months' 
                          AND date_fin_reelle < CURRENT_DATE - INTERVAL '6 months' 
                     THEN cout_total ELSE 0 END) > 0
        THEN (SUM(CASE WHEN date_fin_reelle >= CURRENT_DATE - INTERVAL '6 months' THEN cout_total ELSE 0 END) / 
              SUM(CASE WHEN date_fin_reelle >= CURRENT_DATE - INTERVAL '12 months' 
                          AND date_fin_reelle < CURRENT_DATE - INTERVAL '6 months' 
...
FROM v_maintenance_couts_detaille
WHERE date_fin_reelle >= CURRENT_DATE - INTERVAL '12 months'
GROUP BY 
