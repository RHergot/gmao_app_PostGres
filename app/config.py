...
if dotenv_path:
    logging.debug(f"Fichier .env trouvé : {dotenv_path}")
    # override=True force l'utilisation des valeurs du .env sur les variables système existantes.
    load_dotenv(dotenv_path=dotenv_path, override=True)
else:
    logging.warning("Fichier .env non trouvé. Utilisation des variables d'environnement système ou des valeurs par défaut.")
...
if DATABASE_TYPE in ['postgres', 'postgresql'] and not POSTGRES_PASSWORD:
    raise ValueError(
        "POSTGRES_PASSWORD doit être défini dans le fichier .env "
...
if DATABASE_TYPE in ['postgres', 'postgresql']:
    logging.debug(f"Configuration : DB=PostgreSQL ({POSTGRES_DB}) sur {POSTGRES_HOST}:{POSTGRES_PORT}, LOG_LEVEL={LOG_LEVEL}")
else:
    logging.debug(f"Configuration : DB={DATABASE_TYPE} à {DATABASE_PATH}, LOG_LEVEL={LOG_LEVEL}")
